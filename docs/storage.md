...
用户数据库、证据、导出和浏览器资料不随 npm 发布；测试 fixture 与验收答案也不进入运行包。alpha.5 的系统 ID 和知识继续可读，不因移除固定 SCM 查询工具而删除历史数据。快照产生的控制引用仅用于当前浏览器，不作为跨重启可执行命令保存。

## 数据契约与隔离

`src/storage/contract.ts` 使用宿主原生 Schema，同时推导 TypeScript 类型并校验消息输入/输出。`StorageClient` 通过插件主入口导出；宿主另提供观察回查和知识工具，不开放原始 SQL 或任意文件写入工具。
...
| `rebuildIndex` | 从权威观察和当前知识记录重建派生全文索引，不改观察内容 |
| `check` | SQLite 快速检查、外键检查，以及证据缺失、损坏和孤立文件报告 |
| `backup` / `restore` | 一致性备份与仅向新目录恢复 |

范围使用稳定别名，不能拿登录密码、Cookie 或令牌充当标识。读取、检索、任务更新均按完整范围限定；省略的企业/角色不等同于已确认的企业/角色。观察 ID 全库唯一，生产调用者应生成 UUID，而非以菜单名称或 URL 单独作为 ID。观察当前版本为 1；修改应写新观察并在知识修订中表达关联，不覆盖证据。
...
当前 schema v1 建立观察/证据/任务表，v2 添加观察全文索引，v3 添加知识记录、不可变修订、版本依赖、证据引用、指定命题确认及知识全文索引。已有版本升级前生成并校验完整备份；迁移 SQL 和版本变更在同一事务中完成，失败回滚并保留旧数据及备份。高于当前代码支持的版本直接拒绝。进程意外退出留下的 `.pending-*` 目录不是有效备份，不能拿它恢复；自动清理/保留策略尚未实现。
