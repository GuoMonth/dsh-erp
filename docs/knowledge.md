...
alpha.5：知识范围来自 `erp_system_status` 返回的固定系统及配置身份，不能由模型临时起名；跨对话可在未登录时查询。不同系统目录隔离，旧库不自动导入，详见[系统配置](system-configuration.md)。
...
本增量打通“观察 → AI 解释记录 → 菜单/业务关联 → 查询与修订 → 重启后复用”。解释由已有 dsh Agent 生成，插件负责契约、来源、版本、证据和查询；合成测试中的解释由固定测试数据提供。当前补充固定 SCM 菜单元数据导入与真实模型样本学习、原生关系查询和本地文档导出；全页面自动探索及独立图形编辑界面仍未交付。
