...
本页说明通用人工会话与被动观察。alpha.6 普通用户使用[通用学习入口](adaptive-learning.md)，通过 erp_connect 打开入口，确认后使用包含表格/字段的快照和逐次确认的交互；本页描述保留的窄范围被动标签工具。新增的独立实验入口见 [受控只读导航](controlled-read-navigation.md)，默认未配置，不改变本页人工窗口的操作边界。
...
1. 先按[系统配置](system-configuration.md)设置 URL 和身份别名，调用 `erp_system_status` 核对。普通用户调用无参数 `erp_connect` 打开配置入口；高级被动模式调用无参数 `erp_browser_open` 打开空白窗口，人工导航。模型不能传入或更换 URL/scope。入口支持 hash，内部 Base URL 无 hash/查询串。
...
被动观察工具不提供自动点击、填表、上传、下载、任意 evaluate 或 CDP 操作。普通连接会导航到用户配置的入口；通用快照交互和受审查路线导航另见相应文档。只读指插件开放的操作面；它不是阻止所有网站网络副作用的代理防火墙。人可以在浏览器操作业务，网站自己的脚本也可能发请求；不能仅凭 GET/POST 或本插件标签观察证明网站无写入。Service Worker 当前禁用，但不能据此推断后台请求都被拦截。
