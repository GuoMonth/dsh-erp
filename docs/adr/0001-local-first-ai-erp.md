...
4. 本地工作进程负责浏览器；独立存储线程负责耗时数据库与证据操作，避免浏览器取消中断持久化。模型和宿主审批留在插件侧，通过 IPC 或 stdio 交互，不复制凭据。
5. SQLite 为插件知识与执行记录的权威来源，已在 Node 24.18.0 验证并采用 `node:sqlite`、WAL 和单一所有者；独立文件保存证据，完整备份恢复见 [存储说明](../storage.md)。dsh 会话历史仍由 dsh 管理，不复制出另一份对话真相。
