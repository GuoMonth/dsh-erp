...
async function prepare() {
  ...
  writeFileSync(`${outputDir}/release-notes.md`, `${pkg.name}@${identity.version}\n\nRead-only SCM/USA preview for local DSH 0.1.5-rc.2 and Node >=24.18 <25. A configurable URL does not add compatibility with arbitrary ERPs.\n\nInstall (existing DSH):\n\n\`\`\`sh\nnpm exec --yes --package=pnpm@11.7.0 -- dsh plugin --profile web add ${pkg.name}@${identity.version}\n\`\`\`\n\nBefore connecting, configure system.url in the erp row of your DSH profile cordis.patch.yml and restart DSH. The plugin opens that entry for you to log in manually; approve reads only after confirming the configured account/company/role. Saved knowledge can be queried in later conversations without login; live data needs a current session and read grant.\n\nUpgrade: back up first. Configured systems use separate local stores. Older root-level knowledge remains accessible without system configuration but is not automatically imported. Multi-ERP routing is deferred.\n\n简体中文：先在 DSH profile 配置 ERP 入口，再由用户登录并确认读取。已有知识可跨对话使用；实时查询需有效登录。升级前备份，旧知识保留但不自动迁移。\n\n[English setup](https://github.com/${repository}/blob/${identity.tag}/README.md) · [简体中文](https://github.com/${repository}/blob/${identity.tag}/docs/README.zh-CN.md) · [Configuration and migration](https://github.com/${repository}/blob/${identity.tag}/docs/system-configuration.md) · [Changelog](https://github.com/${repository}/blob/${identity.tag}/CHANGELOG.md)\n\nSource commit: \`${sha}\`. The Action builds and verifies the published package and matching registry/GitHub bytes. Publication does not establish new real ERP, QR/SSO or cross-platform acceptance; see the version validation documents.\n`)
