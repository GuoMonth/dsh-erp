...
已开始 M0 开发，提供可本地打包安装的插件：原生工具、Node 工作进程、宿主模型/审批诊断、SQLite 存储，以及专用 Chromium 会话与被动页面观察。当前开发版本为 `0.1.0-dev.0`，尚未发布到 npm。

已用真实 dsh rc2 CLI 和固定测试模型适配器验证安装、浏览器启动及关闭；本地测试站验证可见导航/控件标签、同站点 iframe、人工接管和观察入库。自动菜单探索、菜单/领域知识模型、真实 ERP 学习验收与业务写入尚未完成。当前验证限于 Linux x64 / Node 24.18.0，容器测试显式关闭浏览器沙箱，产品默认启用。详见 [开发说明](docs/development.md)、[浏览器观察说明](docs/browser-observation.md) 与 [M0 验证记录](docs/m0-runtime-validation.md)。
...
- SQLite 已采用 `node:sqlite`，独立存储线程协调短事务；证据放本地文件。后续知识与关系模型在此基础上扩展。
