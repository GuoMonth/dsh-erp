...
已开始 M0 开发，提供可本地打包安装的插件：原生工具、Node 工作进程、宿主模型/审批诊断、SQLite 存储、专用 Chromium 会话与被动页面观察，以及进入 M1 的版本化菜单/业务知识和原生查询。另提供默认未配置的、按可信契约运行的实验性只读路由读取入口。当前开发版本为 `0.1.0-dev.0`，尚未发布到 npm。
