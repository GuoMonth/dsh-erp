...
已开始 M0 开发，提供可本地打包安装的插件骨架：原生工具注册、Node 工作进程、模型连接诊断、无副作用审批诊断和生命周期清理。当前开发版本为 `0.1.0-dev.0`，尚未发布到 npm。

已用真实 dsh rc2 CLI 和固定测试模型适配器验证安装与运行链路；浏览器、SQLite 知识库、真实 ERP 学习与业务写入尚未实现。当前只验证 Linux x64 / Node 24.18.0，不代表其他平台已兼容。详见 [开发说明](docs/development.md) 与 [M0 验证记录](docs/m0-runtime-validation.md)。
