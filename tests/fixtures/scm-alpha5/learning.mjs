...
export class LearningRuntime {
    ...
    async start(scope, id, units, signal) {
        if (!units.length || units.length > 2000 || new Set(units.map(x => x.id)).size !== units.length || units.some(x => !/^[a-zA-Z0-9][a-zA-Z0-9._:-]{0,127}$/.test(x.id) || !Number.isInteger(x.level) || x.level < 1 || x.level > 4 || !x.label.trim() || x.label.length > 500))
            fail('INVALID_LEARNING_UNITS');
    ...
    async status(scope, id, signal) {
        const task = await this.storage.call('task', { scope, id }, signal);
        if (!task)
            return null;
        const cursor = task.cursor;
        if (cursor?.format !== 1 || !Array.isArray(cursor.units) || !Array.isArray(cursor.settled))
            fail('UNKNOWN_LEARNING_CHECKPOINT');
    ...
    async finish(scope, id, expectedVersion, unitId, outcome, observationIds, reason, signal) {
        const task = await this.status(scope, id, signal);
        if (!task || task.state !== 'running' || task.version !== expectedVersion || task.frontier[0] !== unitId)
            fail('LEARNING_STALE_OR_OUT_OF_ORDER');
        if (!reason.trim() || reason.length > 2000 || observationIds.length > 20 || outcome === 'observed' && !observationIds.length)
            fail('LEARNING_EVIDENCE_REQUIRED');
        for (const observationId of observationIds)
            if (!await this.storage.call('observation', { scope, id: observationId }, signal))
                fail('LEARNING_SCOPE_OR_EVIDENCE');
        const cursor = structuredClone(task.cursor);
        cursor.settled.push({ id: unitId, outcome, observationIds, reason });
        return this.storage.call('checkpoint', { scope, id, expectedVersion, state: task.frontier.length === 1 ? 'round-ended' : 'running', frontier: task.frontier.slice(1), cursor: cursor, reason }, signal);
    }
    async pause(scope, id, expectedVersion, resume, signal) {
        const task = await this.status(scope, id, signal);
        if (!task || task.version !== expectedVersion || !task.frontier.length || !['running', 'paused'].includes(task.state))
            fail('LEARNING_STALE_OR_ENDED');
    ...
    async importMenu(scope, observationId, signal) {
        const observation = await this.storage.call('observation', { scope, id: observationId }, signal);
        if (!observation || JSON.parse(observation.context).source !== 'scm-usa-read-v1' || JSON.parse(observation.context).query !== 'menu')
            fail('MENU_OBSERVATION_REQUIRED');
        const rows = JSON.parse(observation.text);
        const nodes = [];
        const seen = new Set();
        const visit = (list, parent = '', level = 1) => {
            if (!Array.isArray(list) || level > 8)
                fail('INVALID_MENU_TREE');
            for (const row of list) {
                if (!row || typeof row.id !== 'string' || seen.has(row.id) || typeof row.name !== 'string' || typeof row.url !== 'string' || nodes.length >= 2000)
                    fail('INVALID_MENU_TREE');
                seen.add(row.id);
        ...
        const put = async (record) => {
            ...
            if (current?.record.origin === 'user')
                return current.record.version;
            // A retry against the same source observation does not create another version.
            if (current && current.record.evidence.some(x => x.observationId === observationId) && JSON.stringify(current.record.dependencies) === JSON.stringify([...record.dependencies, ...(record.from ? [record.from] : []), ...(record.to ? [record.to] : [])]))
                return current.record.version;
        ...
        for (const [kind, query, prefix, desired] of [
            ...
            do {
                ...
                for (const { record } of page.items)
                    if (record.id.startsWith(prefix) && record.origin === 'ai' && record.lifecycle === 'active' && !desired.has(record.id)) {
